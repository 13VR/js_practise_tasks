...
taskTitle.innerText = "Task 10: Built-in object capabilities ";
...
function customObjectFromEntries(entries) {
  ...
  const mappedValue = entries.map(([key, value]) => {
    if (typeof key === "number") {
      value = key.toString();
    }
    return [key, value];
  });
  const newObj = Object.fromEntries(mappedValue);
  return newObj;
...
function filterObjectsByKey(arr, key) {
  ...
  const newArray = arr.filter((el) => el.hasOwnProperty(key));
  return newArray;
...
function checkOwnership(obj1, obj2, key) {
  if (obj1.hasOwnProperty(key) && obj2.hasOwnProperty(key)) {
    const same = Object.is(obj1.value, obj2.value);
    if (same === true) {
      return true;
    } else {
      return false;
...
function getObjectValuesSum(obj) {
  ...
  const sum = newObj.reduce((accum, currentValue) => accum + currentValue);
  return sum;
...
const result1 = Object.groupBy(inventory, myCallback);
console.log(result1);
