...
taskTitle.innerText = "Task 2: Functions";
...
function getIntegerFromString(s) {
  ...
  let result;
  if (isNaN(number)) {
    result = "Couldn't extract an integer from the provided string.";
  } else {
    result = number;
  }
...
function decodeURLComponent(urlComponent) {
  let decodeUrl = decodeURIComponent(urlComponent);
  return decodeUrl;
...
function decodeURL(url) {
  let decodeUrlComponent = decodeURI(url);
