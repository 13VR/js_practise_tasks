...
taskTitle.innerText = "Task 13: JSON & URL ";
...
function searchParamsURL(url) {
  const newUrl = new URL(url);
  const search = newUrl.searchParams;
  console.log(search);
...
function manipulateSearchParams(paramsObj, newUrl) {
  ...
  const allKeys = Object.keys(paramsObj);
  ...
  for (const [key, value] of options) {
    if (key === "append") {
      for (const [key1, value1] of value) {
        newUrl.searchParams.set(key1, value1);
      }
    }
    if (key === "delete") {
...
function getURLValues(url) {
  ...
  const array = Array.from(URLSearchParams.keys());
  const values = [];
  for (const key of array) {
    const elem = URLSearchParams.getAll(key);
    values.push(...elem);
  }
  return values;
