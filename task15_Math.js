...
taskTitle.innerText = "Task 15: Math ";
// Завдання 1
/**
...
function findMinMax(numbers) {
  if (!Array.isArray(numbers)) {
    console.log("Error, it shoudl be array");
    return null;
  }
...
function calculateHypotenuse(a, b) {
  if (typeof a === "number" || typeof b === "number  ") {
    const gipotenusa = Math.sqrt(Math.pow(a, 2) + Math.pow(b, 2));
    return gipotenusa;
  } else {
    console.log("Error");
    return null;
...
function roundObjectValues(obj) {
  if (typeof obj === "object" && obj !== null) {
    ...
    const rounded = array.map(([key, value]) => {
    ...
    const newObj = Object.fromEntries(rounded);
    return newObj;
  } else {
...
function getFractionalPart(num) {
  ...
  const rest = num - trunc;
  const result = Math.fround(rest);
  return result;
...
function estimateInvestment(principal, interestRate, years) {
  ...
  const result = principal * Math.pow(1 + interestRate, years);
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  const totalPrice = products.reduce((accum, price) => accum + price.price, 0);
