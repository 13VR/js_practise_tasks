...
taskTitle.innerText = "Task 9: Built-in array capabilities PART 2";
...
function findElementGreaterThan(arr, num) {
  ...
  const newNum = arr.find((el) => {
    if (el > num) {
      return el;
    }
  });
  return newNum || "Числа яке задовільняе умову не знайдено";
...
function findIndexByProperty(arr, name) {
  ...
  const findEl = (el) => el.name === name;
  const newIndex = arr.findIndex(findEl);
  return newIndex;
  // return newIndex;
...
function findLastEvenNumber(arr) {
  ...
  const lastNumber = arr.findLast((el) => el % 2 === 0);

  return lastNumber || "Парних чисел не знайдено";

  // const lastNumber = arr.findLast((el) => {
  //   if (el % 2 === 0) {
  //     console.log(el);
  //     return el;
  //   }
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  const sliced = arr.slice(startIdx).join(" ");
  return sliced;
...
function getPositiveArrayValues(arr) {
  ...
  return Array.from(iterator).filter((el) => el > 0);
...
function removeAndDouble(arr, startIdx) {
  ...
  const newArr = arr.splice(0, startIdx).map((el) => el * 2);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const flatedArray = arr.flat();
  const squaredArray = flatedArray.map((el) => el ** 2);
  console.log(squaredArray);
  return squaredArray.filter((el) => el > threshold);
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const slicedArray = arr.slice(n + 1);
  console.log(slicedArray);
  const reducedEl = slicedArray.reduceRight(
    (accumulator, currentValue) => accumulator + currentValue
  );
  slicedArray.forEach((el) => console.log(el));
  return reducedEl;
  // console.log(reducedEl);
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function wordLengths(arr) {
  ...
  const lengthsArray = arr.map((el) => el.length);
  return lengthsArray;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо 'Вхідний параметр має бути масивом'
...
function expandArrayByN(arr, n) {
  ...
  const flattedArray = arr.flatMap((el) => {
...
function findLongestWord(arr) {
  ...
  const longestWord = arr.reduce((longest, currentValue) => {
...
function findDuplicateElements(arr) {
  ...
  const filteredArray = arr.filter((el, index) => arr.indexOf(el) !== index);
...
function capitalizeWords(arr) {
  ...
  const newArray = arr.map((el) => {
    const slicedWord = el.slice(0, 1).toUpperCase();
    const rest = el.slice(1);
    return slicedWord + rest;
  });
  return newArray;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо пустий масив
...
function calculateTotalPrice(arr) {
  ...
  const sum = 0;
  const result = arr.reduce((accum, currentValue) => {
