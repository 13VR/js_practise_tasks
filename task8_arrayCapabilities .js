...
taskTitle.innerText = "Task 8: Built-in array capabilities";
...
function getUsersFromDatabase(array) {
  const newArray = Array.from(array, (user) => ({
    id: user.id,
...
function filterByCondition(arr, condition) {
  const newArr = arr.filter(condition);
  return newArr;
...
function combineAndReverseArrays(arr1, arr2) {
  if (Array.isArray(arr1) && Array.isArray(arr2)) {
    const newArr = arr1.concat(arr2).reverse();
    return newArr;
  } else {
    return [];
  }
...
function copyAndSwapElements(arr, target, start, end) {
  ...
  const newArr = arr.copyWithin(target, start, end);
  return newArr;
...
function customEvery(arr, condition) {
  ...
  const newArr = arr.every(condition);
  return newArr;
...
function customIncludes(arr, element) {
  if (Array.isArray(arr)) {
    const el = arr.includes(element);
    const amount = arr.filter((word) => {
      if (word === element) {
        return word;
      }
    });
