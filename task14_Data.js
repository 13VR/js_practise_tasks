...
taskTitle.innerText = "Task 14: Data ";
...
function setDateDetails(date, isoString) {
  ...
  if (!typeof newDate.getDate() === "number") {
...
function compareDates(date1, date2) {
  ...
  const data1ToIso = date1.toISOString();
  const data2ToIso = date2.toISOString();
  ...
  return { data1ToIso, data2ToIso, comparison };
...
function getDaysDifference(startDate, endDate) {
  ...
  const startToIso = new Date(start).toISOString();
  const endToIso = new Date(endDate).toISOString();

  return { startToIso, endToIso, daysDifference };
  // Перевірка, чи є вхідні значення об'єктами Date.
