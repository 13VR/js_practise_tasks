...
taskTitle.innerText = "Task 16: Console ";
// Завдання 1
...
function compareArrays(arr1, arr2) {
  ...
  const equalArr = JSON.stringify(sortedArr1) === JSON.stringify(sortedArr2);
  console.assert(equalArr, "Масиви не однакові!");
...
function calculateTotalPrice(products) {
  console.time("timer");
  const total = products.reduce((sum, el) => sum + el.price, 0);
...
function countVowelsAndConsonants(word) {
  ...
  for (const letter of word) {
    if (vowel.includes(letter.toLowerCase())) {
      letterCount1++;
      console.count(`Голосні: ${letterCount1}`);
    } else {
      letterCount2++;
      console.count(`Приголосні: ${letterCount2}`);
    }
  }
  console.countReset();
